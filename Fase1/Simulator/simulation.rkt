    ...
    (let while ()
      ...
      (unless (< dx (if forward?
        ...
        (define n3 (if forward?
                       (next-node n1 n2)
                       (next-node n2 n1)))
        (unless n3     ;node3 bestaat niet dus het rijden naar node3 betekent dat we rijden naar iets zonder spoor
...
(define (other-locos-track-pos t1 lid)
  ;;Deze lijst bewaart voor al de locos die op dezelfde spoor liggen van lid
  ;;de afstand van de andere loco tot 
  (define others '())  
...
(define (next-node n1 n2)
  ...
  ;;;Je hebt in totaal 4 mogelijkheden als je loco via node2 naar node 3 rijdt
  ;;1. Je moet van spoor veranderen via een switch dus node2 is een switch en je loco zit op een switch
  ;;2. Je rijdt naar een gewoon track waar dus een normale verbinding bestaat tussen n2 en n3
  ;;3. Je rijd naar een switch toe
  (define swx
    (findf (lambda (s)
  ...
  (cond [swx
         ...
         ;Hier returnt men rechtstreeks een node nl n1 of n2, want we veronderstellen dat de switch al op de correcte
         ;position staat 1 of 2 om naar de volgende node te gaan
        ...
        [else
         ...
         (let ([tracks '()])
           ...
           (and (= (length tracks) 2)
                ...
                ;;Er kunnen enkel twee tracks zijn van de vorm (track node node2) of (track node2 node)
                ;;De enige manier dat het anders is is dat n2 een switch is (track n2 n0), (track n2 n1) (track n2 n3) 
